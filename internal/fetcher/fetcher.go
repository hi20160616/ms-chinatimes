...
func Fetch() error {
	defer log.Printf("[%s] Fetch Done.", configs.Data.MS["chinatimes"].Title)
	log.Printf("[%s] Fetching ...", configs.Data.MS["chinatimes"].Title)
			...
			if err != nil {
				if errors.Is(err, ErrTimeOverDays) ||
					...
					log.Printf("[%s] predictability error: %v, link: %s",
						configs.Data.MS["chinatimes"].Title, err, link)
				} else {
